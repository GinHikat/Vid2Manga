...
def get_gcs_client(creds_path: str = google_api_creds):
    """
    Authenticate and return a Google Cloud Storage client object.
    """
    creds = service_account.Credentials.from_service_account_file(creds_path)
    return storage.Client(credentials=creds, project=creds.project_id)

def upload_file_to_gcs(local_path: str, bucket_name: str, destination_blob_name: Optional[str] = None, creds_path: str = google_api_creds):
    """
    Uploads a file to a GCS bucket.
    """
    client = get_gcs_client(creds_path)
...
def download_file_from_gcs(bucket_name: str, source_blob_name: str, local_destination_path: Optional[str] = None, creds_path: str = google_api_creds):
    """
    ...
    if local_destination_path is None:
        local_destination_path = os.path.basename(source_blob_name)
        
    blob.download_to_filename(local_destination_path)
    print(f"✅ File {source_blob_name} from bucket {bucket_name} downloaded to {local_destination_path}.")
    return local_destination_path

def list_gcs_files(bucket_name: str, prefix: Optional[str] = None, creds_path: str = google_api_creds):
    """
...
def delete_gcs_file(bucket_name: str, blob_name: str, creds_path: str = google_api_creds):
    """
